  ...
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        #     np.random.shuffle(blobs['crowdsourced_classes'])
            crowdsourced_classes = torch.from_numpy(blobs['crowdsourced_classes'])
        else:
        ...
        gt_boxes_padding = torch.LongTensor(self.max_num_box, gt_boxes.size(1)).zero_()
        # 填充 crowdsourced_classes
        if cfg.LABEL_SOURCE == 2:
            padding_crowdsourced_classes = torch.FloatTensor(self.max_num_box, cfg.NUM_ANNOTATOR).zero_()
        if keep.numel() != 0:
            gt_boxes = gt_boxes[keep]
            num_boxes = min(gt_boxes.size(0), self.max_num_box)
            gt_boxes_padding[:num_boxes,:] = gt_boxes[:num_boxes]
            if cfg.LABEL_SOURCE == 2:
                padding_crowdsourced_classes[:num_boxes] = crowdsourced_classes[:num_boxes]
        else:
            num_boxes = 0
